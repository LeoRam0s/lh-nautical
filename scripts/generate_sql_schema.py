    ...
    with open(
        ...
        for row in reader:
            for header in headers:
                ...
                if (
                    TYPE_PRIORITY[detected_type]
                    > TYPE_PRIORITY[current_type]
                ):
                    column_types[header] = detected_type
...
def generate_create_table(file_path):
    headers, column_types = infer_columns(file_path)
...
def generate_schema(
    ...
    for file_name in sorted(os.listdir(csv_directory)):
        ...
        create_table = generate_create_table(file_path)
